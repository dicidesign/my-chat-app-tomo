        ...
        try {
                if (data.createdAt && data.createdAt.seconds) {
                    ...
                    if (messageDateString !== lastMessageDate) {
                        ...
                        const dayOfWeek = ['日', '月', '火', '水', '木', '金', '土'][messageDate.getDay()];
                        dateStamp.textContent = `${messageDate.getFullYear()}年${messageDate.getMonth() + 1}月${messageDate.getDate()}日 (${dayOfWeek})`;
                        messages.appendChild(dateStamp);
                        lastMessageDate = messageDateString;
                    }
                }
            } catch (e) {
                console.error("日付処理でエラー:", e);
            }
            // --- ↑↑↑ ここまでが日付処理の追加部分 ---
