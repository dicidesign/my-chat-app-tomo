...
const displayMessage = (data) => {
    ...
    Swal.fire({
        ...
        // 2. OKボタンを「ダウンロード」ボタンとして活用する
        showConfirmButton: true,
        confirmButtonText: '<i class="fas fa-download"></i> ダウンロード', // Font Awesomeのアイコンを使う
        confirmButtonAriaLabel: 'ダウンロード',

        // 3. キャンセルボタンは不要なので非表示に
        showCancelButton: false,

        // 4. ダウンロードボタンが押された時の処理
        preConfirm: () => {
            // aタグを作成して、ダウンロードを実行する
            const link = document.createElement('a');
            link.href = img.src;
            // download属性にファイル名を指定（ファイル名が取得できないので、一般的な名前に）
            link.download = `chat-image-${Date.now()}.jpg`; 
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
            return false; // preConfirmの後、アラートが閉じるのを防ぐ
        },
                customClass: {
                    popup: 'fullscreen-swal' // 専用のCSSクラスを当てる
                }
            });
        });
