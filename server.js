...
// server.js の io.on(...) の前あたりに追加

// --- ↓↓↓ ここからがダウンロード用API ↓↓↓ ---
const axios = require('axios'); // HTTPリクエストを簡単にするライブラリ

// '/download-image' というURLへの問い合わせに対応
app.get('/download-image', async (req, res) => {
    const imageUrl = req.query.url; // クエリパラメータから画像のURLを取得
    if (!imageUrl) {
        return res.status(400).send('Image URL is required');
    }

    try {
        // Axiosで画像データをバッファとして取得
...
socket.on('chat message', async (data) => {
  ...
  const messageToStore = {
    ...
    createdAt: new Date(),
    isImage: data.isImage || false // ←←← この行を追加！
  };
